...
void Umier::ProcessPe(neoReference &r1, neoReference &r2) {
    ...
    if (cmd_info_->umi_loc_ == UMI_LOC_INDEX1) {
        ...
        int trim_len1 = std::min(seq_len1 - 1, int(umi.length() + cmd_info_->umi_skip_));
        r1.pseq += trim_len1;
        r1.pqual += trim_len1;
        r1.lseq -= trim_len1;
        r1.lqual -= trim_len1;
        ...
        int trim_len2 = std::min(seq_len2 - 1, int(umi.length() + cmd_info_->umi_skip_));
        r2.pseq += trim_len2;
        r2.pqual += trim_len2;
        r2.lseq -= trim_len2;
        r2.lqual -= trim_len2;
...
void Umier::addUmiToName(neoReference &ref, std::string umi) {
    ...
    if (spacePos == -1) {
        ...
        ref.pname = reinterpret_cast<uint64_t>(new_name);
    } else {
        char *new_name = new char[ref.lname + tag.length()];
        int pos = 0;
