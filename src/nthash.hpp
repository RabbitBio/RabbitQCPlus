...
static const uint64_t A31l[31] = {
        ...
        0x917f764e00000000, 0x22feec9e00000000, 0x45fdd93c00000000, 0x8bfbb27800000000, 0x17f764f200000000,
        0x2feec9e400000000, 0x5fdd93c800000000, 0xbfbb279000000000, 0x7f764f2200000000, 0xfeec9e4400000000,
...
static const uint64_t C31l[31] = {
        ...
        0x3278308c00000000, 0x64f0611800000000, 0xc9e0c23000000000, 0x93c1846200000000, 0x278308c600000000,
        0x4f06118c00000000, 0x9e0c231800000000, 0x3c18463200000000, 0x78308c6400000000, 0xf06118c800000000,
...
static const uint64_t G31l[31] = {
        ...
        0x647da0800000000, 0xc8fb41000000000, 0x191f682000000000, 0x323ed04000000000, 0x647da08000000000,
        0xc8fb410000000000, 0x91f6820200000000, 0x23ed040600000000, 0x47da080c00000000, 0x8fb4101800000000,
...
static const uint64_t T31l[31] = {
        ...
        0x2aa93e8a00000000, 0x55527d1400000000, 0xaaa4fa2800000000, 0x5549f45200000000, 0xaa93e8a400000000,
        0x5527d14a00000000, 0xaa4fa29400000000, 0x549f452a00000000, 0xa93e8a5400000000, 0x527d14aa00000000,
...
inline uint64_t NTF64(const char *kmerSeq, const unsigned k) {
    ...
    for (unsigned i = 0; i < k / 4; i++) {
        ...
        uint8_t tetramerLoc = 64 * convertTab[(unsigned char) kmerSeq[currOffSet]] +
                              16 * convertTab[(unsigned char) kmerSeq[currOffSet + 1]] +
                              4 * convertTab[(unsigned char) kmerSeq[currOffSet + 2]] +
                              convertTab[(unsigned char) kmerSeq[currOffSet + 3]];
    ...
    if (remainder == 3) {
        uint8_t trimerLoc =
                16 * convertTab[(unsigned char) kmerSeq[k - 3]] + 4 * convertTab[(unsigned char) kmerSeq[k - 2]] +
                convertTab[(unsigned char) kmerSeq[k - 1]];
...
inline uint64_t NTR64(const char *kmerSeq, const unsigned k) {
    ...
    if (remainder == 3) {
        uint8_t trimerLoc =
                16 * RCconvertTab[(unsigned char) kmerSeq[k - 1]] + 4 * RCconvertTab[(unsigned char) kmerSeq[k - 2]] +
                RCconvertTab[(unsigned char) kmerSeq[k - 3]];
        hVal ^= trimerTab[trimerLoc];
    } else if (remainder == 2) {
        uint8_t dimerLoc =
                4 * RCconvertTab[(unsigned char) kmerSeq[k - 1]] + RCconvertTab[(unsigned char) kmerSeq[k - 2]];
        hVal ^= dimerTab[dimerLoc];
    } else if (remainder == 1) {
    ...
    for (unsigned i = 0; i < k / 4; i++) {
        ...
        uint8_t tetramerLoc = 64 * RCconvertTab[(unsigned char) kmerSeq[currOffSet]] +
                              16 * RCconvertTab[(unsigned char) kmerSeq[currOffSet - 1]] +
                              4 * RCconvertTab[(unsigned char) kmerSeq[currOffSet - 2]] +
                              RCconvertTab[(unsigned char) kmerSeq[currOffSet - 3]];
        hVal ^= tetramerTab[tetramerLoc];
    }
    return hVal;
}
...
inline uint64_t
NTF64L(const uint64_t rhVal, const unsigned k, const unsigned char charOut, const unsigned char charIn) {
...
inline uint64_t
NTR64L(const uint64_t fhVal, const unsigned k, const unsigned char charOut, const unsigned char charIn) {
    uint64_t hVal = rol1(fhVal);
    hVal = swapbits033(hVal);
...
inline uint64_t
NTC64L(const unsigned char charOut, const unsigned char charIn, const unsigned k, uint64_t &fhVal, uint64_t &rhVal) {
...
inline void
NTM64(const unsigned char charOut, const unsigned char charIn, const unsigned k, const unsigned m, uint64_t *hVal) {
    uint64_t bVal = 0, tVal = 0;
...
inline void
NTMC64(const char *kmerSeq, const unsigned k, const unsigned m, uint64_t &fhVal, uint64_t &rhVal, uint64_t *hVal) {
    uint64_t bVal = 0, tVal = 0;
...
inline void
NTMC64(const unsigned char charOut, const unsigned char charIn, const unsigned k, const unsigned m, uint64_t &fhVal,
       uint64_t &rhVal, uint64_t *hVal) {
    uint64_t bVal = 0, tVal = 0;
...
// canonical ntHash
inline bool
NTC64(const char *kmerSeq, const unsigned k, uint64_t &fhVal, uint64_t &rhVal, uint64_t &hVal, unsigned &locN) {
    hVal = fhVal = rhVal = 0;
    locN = 0;
    for (int i = k - 1; i >= 0; i--) {
        if (seedTab[(unsigned char) kmerSeq[i]] == seedN) {
...
// canonical multihash ntHash
inline bool
NTMC64(const char *kmerSeq, const unsigned k, const unsigned m, uint64_t &fhVal, uint64_t &rhVal, unsigned &locN,
       uint64_t *hVal) {
    fhVal = rhVal = 0;
    uint64_t bVal = 0, tVal = 0;
    locN = 0;
...
inline void
NTMC64(const unsigned char charOut, const unsigned char charIn, const unsigned k, const unsigned m, uint64_t &fhVal,
       uint64_t &rhVal, uint64_t *hVal, bool &hStn) {
    uint64_t bVal = 0, tVal = 0;
    bVal = NTC64(charOut, charIn, k, fhVal, rhVal);
    hStn = rhVal < fhVal;
    hVal[0] = bVal;
...
inline uint64_t
NTS64(const char *kmerSeq, const std::vector<bool> &seed, const unsigned char charOut, const unsigned char charIn,
      const unsigned k, uint64_t &hVal) {
