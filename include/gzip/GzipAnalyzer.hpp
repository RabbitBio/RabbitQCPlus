    ...
    while ( true ) {
        ...
        case FileType::GZIP:
            if ( !gzipHeader ) {
                ...
                if ( error != Error::NONE ) {
                    std::cout << "Encountered error: " << toString( error ) << " while trying to read gzip header!\n";
                    return error;
                }
        ...
        case FileType::ZLIB:
            if ( !zlibHeader ) {
                ...
                if ( error != Error::NONE ) {
                    std::cout << "Encountered error: " << toString( error ) << " while trying to read zlib header!\n";
                    return error;
                }
        ...
        const auto blockOffset = bitReader.tell();
        {
            ...
            if ( error != Error::NONE ) {
                std::cout << "Encountered error: " << toString( error )
        ...
        while ( !block.eob() ) {
            ...
            if ( error != Error::NONE ) {
                std::cout << "Encountered error: " << toString( error ) << " while decompressing deflate block.\n";
            }
        ...
        case FileType::GZIP:
        {
            ...
            try {
                if ( crc32Calculator.verify( footer.crc32 ) ) {
                    std::cout << "Validated CRC32 0x" << std::hex << crc32Calculator.crc32() << std::dec
                              << " for gzip stream.\n";
                }
            } catch ( const std::domain_error& exception ) {
                std::cout << "CRC32 validation for gzip stream failed with: " << exception.what() << "\n";
            }
