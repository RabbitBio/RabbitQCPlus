...
private:
                              ...
                              const FasterVector<uint8_t>&                     lastWindow )
    {
        ...
        for ( const auto& subchunk : subchunks ) {
            ...
            if ( subchunk.window ) {
                ...
                std::cout << "[Info] The subchunk window for offset " << windowOffset << " is not compressed yet. "
              ...
              const size_t blockIndex )
    {
        ...
        if ( BaseType::statisticsEnabled() ) {
            ...
            {
                std::cout << "[Info] Detected a performance problem. Decoding might take longer than necessary. "
                  ...
                  && ( partitionOffset != blockOffset ) ) )
        {
            ...
            {
                std::cout << "Unexpected end of file when getting block at " << formatBits( blockOffset )
