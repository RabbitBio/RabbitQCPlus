#ifndef CARE_MMAP_BUFFER_HPP
#define CARE_MMAP_BUFFER_HPP
...
private:
    ...
    void grow(std::size_t mincapacity){
        constexpr int growth = 2;
        if(mincapacity <= capacity() * growth){
            buffer.resize(sizeof(T) * capacity() * growth);
            capacity_ = capacity() * growth;
        }else{
