...
public:
    ...
    ~ParallelGzipReader() override
    {
        if ( m_showProfileOnDestruction && m_statisticsEnabled ) {
            std::cout << "[ParallelGzipReader] Time spent:";
    ...
    gzipIndex( bool withLineOffsets = false )
    {
        ...
        if ( !archiveSize && !m_indexIsImported ) {
            ...
            std::cout << "[Warning] The input file size should have become available after finalizing the index!\n";
            std::cout << "[Warning] Will use the last chunk end offset as size. This might lead to errors on import!\n";
        }
...
public:
    ...
    importIndex( UniqueFileReader indexFile )
    {
        ...
        if ( m_showProfileOnDestruction ) {
            std::cout << "[ParallelGzipReader::importIndex] Took " << duration( t0 ) << " s\n";
        }
    }
                 ...
                 const IndexFormat                                             indexFormat = IndexFormat::INDEXED_GZIP )
    {
        ...
        if ( m_showProfileOnDestruction ) {
            std::cout << "[ParallelGzipReader::exportIndex] Took " << duration( t0 ) << " s\n";
        }
    }
              ...
              const size_t                                 dataToWriteSize )
            {
                ...
                while ( it != newlineOffsets.end() ) {
                    ...
                    if ( !chunkInfo.contains( *it ) ) {
                        ...
                        std::cout << "[Warning] Offset in processed chunk was not found in chunk map!\n";
                        break;
                    }
